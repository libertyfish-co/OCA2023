...
+ [独自機能を考えて追加してみよう](#独自機能を考えて追加してみよう)
  + [キャラクターの生存状況を表示](#キャラクターの生存状況を表示)
  + [回復魔法](#回復魔法)
  + [魔法こうげき（強化）](#魔法こうげき（強化）)
  + [命中率](#命中率)
  + [コンティニュー](#コンティニュー)
...
## 独自機能を考えて追加してみよう
ここまで基本的な部分を作ってきました。より良いゲームに仕上げるために、応用として独自の機能を作ってみましょう！以下はそのサンプルです。
...
## おまけ
本テキストで学習した内容を活用し、さらに独自の機能を考えて追加したコードです。  
気になる点があれば、ぜひ参考にしてみてください。  

このコードの特徴は以下のとおりです。  

+ 生存状況をマークで表示
+ Messageクラスでメッセージの表示を一元管理
+ 画面の文字に色を付ける独自機能を追加
+ メッセージの表示間隔を調整
+ メッセージ出力用のメソッドを統一
+ 三項演算子を活用し、簡潔な記述を意識
+ 一部の変数や定数を短く定義し、可読性を向上
