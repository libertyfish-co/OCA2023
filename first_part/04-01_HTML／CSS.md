...
| タグ | 説明 |
|---|---|
| `<h1>～<h6>` | 見出しタグ。数字が小さいほど**重要度が高い**。      |
| `<p>` | **段落**タグ。テキストを段落として表示。             |
| `<a href="URL">` | **リンク**タグ。`href`属性でリンク先のURLを指定。    |
| `<ul>`, `<ol>`, `<li>` | **リスト**タグ。`<ul>`は順不同リスト、`<ol>`は順序付きリスト、`<li>`はリスト項目。 |
| `<img src="画像のURL" alt="画像の説明">` | **画像**タグ。`src`で画像のURL、`alt`で画像の説明を指定。 |
