...
`@heroes`を渡しているところは**各キャラのオブジェクト**を渡せばいいので、`character`に書き換えます。
...
when Constants::ACTION_ATTACK
  ...
  if target_monster   # オブジェクトが入っている場合はtrueになる
...
+ `target_monster = @monsters.select(&:is_alive).sample`  
  `is_alive`が`true`になっているオブジェクトを`select`で絞り込みます。その後、`sample`でこうげき対象となるオブジェクトをランダムに1つ取り出します。  
...
ループを抜けた後に勝敗判定をして、メッセージを表示させましょう。
