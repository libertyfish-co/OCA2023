...
また、以下のように記述すると引数を受け取らなかった場合に自動的に代入させることもできます。
...
show_message("Good Morning!") #=> Good Morning!
show_message()                #=> Hello!
```
...
#### `round`
浮動小数点数を指定した桁数で四捨五入するメソッド。引数を指定しない場合は、最も近い整数に丸められます。
```ruby
num = 3.14159
result = num.round(2)  # 小数点以下2桁に四捨五入
puts result  # 結果は 3.14
...
#### `ceil`
浮動小数点数を指定した桁数で切り上げるメソッド。引数を指定しない場合は、最も近い整数に切り上げます。
```ruby
num = 3.14159
result = num.ceil(2)  # 小数点以下2桁で切り上げ
puts result  # 結果は 3.15
...
#### `floor`
浮動小数点数を指定した桁数で切り下げるメソッド。引数を指定しない場合は、最も近い整数に切り下げます。
```ruby
num = 3.14159
result = num.floor(2)  # 小数点以下2桁で切り下げ
puts result  # 結果は 3.14
...
`object.method`という形式で、オブジェクトのメソッドを呼び出します。
