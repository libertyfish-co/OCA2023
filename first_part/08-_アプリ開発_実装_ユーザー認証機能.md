...
2つ目は最初に編集した application_controller.rb です。このコントローラーにログイン後、ログアウト後の遷移先を設定します。  
...
<div class="field">
  <%= f.label :name %><br>
  <%= f.text_field :name, autocomplete: "email" %>
</div>
<!-- 追加　ここまで -->
...
```
ちなみに `<%= f.label :email %>` などですが、：（コロン）の後ろを編集することで、表示させたいタイトルを表示させることができます。  
