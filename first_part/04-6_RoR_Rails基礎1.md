...
* **開発の土台を作ってくれる**  
  フレームワークは、家の「地盤の基礎工事と骨組み」という基本的な部分を作ってくれます。

* **見た目や機能を作るだけ**  
  開発者は、基本的な構造や機能が整った状態から始め、家でいう「外装（デザイン）」や「内装（機能）」を作成するだけなので、効率よく開発を進めることができます。
...
それではRailsを使って実際にアプリケーションを作成してみましょう。
...
1. **ターミナルでrails newコマンドでアプリケーション枠組みの作成**  
   画像のようにコマンドを打ち込んでください

1. **rails db:createコマンドを実行**
     画像のようにコマンドを打ち込んでください
   
1. **rails g scaffold コマンドでリソースを一括作成**
     画像のようにコマンドを打ち込んでください
...
1. **ブラウザで「 http://localhost:3000 」にアクセス**
   ブラウザ（chromeなど）を開いてURLを検索欄に貼り付けてください

1. **URLに「User」を入れてください**
   画面が出来上がっています
