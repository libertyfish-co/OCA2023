...
- **Docker**  
  アプリケーションをコンテナという独立した環境で実行するためのツールです。  
  これにより、開発環境や本番環境の違いを気にせず、どこでも同じ環境でアプリケーションを実行できます。
...
ディレクトリの中に新たなディレクトリを作って分類し管理する方法を「**階層構造**/**ツリー構造**」と呼びます。

<img src="images/Rails基礎/ディレクトリ概念.png" width= "700px">

> メモ ： `cd` コマンドはディレクトリ間を移動するためのコマンドです。

ルートディレクトリなどはファイルやディレクトリの位置をわかりやすく示したものですが、他にもファイルやディレクトリまでの**経路**を示す「**パス**」という概念があります。

#### パスとは？

パスは、ファイルやディレクトリがどこに存在するかを指し示すもので、絶対パスと相対パスの2種類があります。



アプリの作成では **CLI 「コマンドラインインターフェース（Command Line Interface）」** を使って作業します。  
...
* **ホームディレクトリ**  
  あなた専用の作業スペースで、/home/ユーザー名 がこれになります。  
  ターミナルでは、~（チルダ） で表示されていて、 /home/ユーザー名 と同じ意味です。  
  アプリのルートディレクトリも、このホームディレクトリ内に作られることが多いですよ。
