...
* **HTTP/HTTPS** とは  
  * 「**HTTP**や**HTTPS**は、それぞれ(**HyperText Transfer Protocol**)、HTTPS(**HyperText Transfer Protocol Secure**)
の略称です。」
  * WebブラウザとWebサーバが通信するためのプロトコルで、主にWebサイト表示に使われます。  
  最も大きな違いは**通信の安全性**で、URLの「**http://**」は暗号化されていない通信、そして「**https://**」は暗号化されていることを示します。  
...
* **FTP**とは  
   * 「**FTP**(**File Transfer Protocol**)の略称です。」
   * ファイルを転送するための通信規格です。  
...
* **TCP**とは  
  * 「**TCP**(**Transmission Control Protocol**)の略称です。」
...
* **UDP**とは  
  * 「**UDP**(**User Datagram Protocol**)の略称です。」
