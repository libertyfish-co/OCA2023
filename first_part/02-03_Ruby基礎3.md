...
+ `date`  
  ...
  <details>
  <summary>Dateクラスの主な使い方</summary>

  #### `Date` クラス
  Rubyの標準ライブラリで提供される `Date` クラスを使用すると、日付を扱うことができます。

  - 日付の取得
  ```ruby
  require 'date'
  today = Date.today
  puts today  # 結果は今日の日付（例: 2025-02-06）
  ```

  - 特定の日付を作成
  ```ruby
  date = Date.new(2022, 12, 25)  # 2022年12月25日を作成
  puts date  # 結果は 2022-12-25
  ```
  ...
  next_day = date + 1  # 1日後
  puts next_day  # 結果は 2022-12-26

  previous_day = date - 1  # 1日前
  puts previous_day  # 結果は 2022-12-24
  ```

  - 日付のフォーマット
  ```ruby
  date = Date.new(2022, 12, 25)
  formatted_date = date.strftime("%Y/%m/%d")  # フォーマットを変更
  puts formatted_date  # 結果は 2022/12/25
  ```
  </details>
